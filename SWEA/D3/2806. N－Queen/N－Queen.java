...
//		System.setIn(new FileInputStream("res/input.txt"));
	...
	static void choose(int depth) {
		...
		for (int i = 0; i < N; i++) {
			...
			boolean isDiagonalDuplicate = isDiagonalDuplicate(depth, i);

			// 하나라도 겹치면(가로, 세로, 대각선) continue
			if(garo[depth] || sero[i] || isDiagonalDuplicate)
				continue;
	...
	static boolean isDiagonalDuplicate(int i, int j) {
